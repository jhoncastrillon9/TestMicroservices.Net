...
namespace MS.AFORO255.Deposit.Services
{
    public class AccountService : IAccountService
    {
        ...
        public bool Execute(Models.Transaction request)
        {
            bool response = false;
            ...
            retry.Execute(() =>
            {
                if (circuitBreakerPolicy.CircuitState == CircuitState.Closed)
                {
                    circuitBreakerPolicy.Execute(() =>
                    {
                        ...
                        response = TransacctionAccount(account).Result;
                    });
                }
                ...
                if (circuitBreakerPolicy.CircuitState != CircuitState.Closed)
                {
                    ...
                    TransacctionReverse(transaction);
                    response = false;
                }
            });
        ...
        public async Task<bool> TransacctionAccount(AccountRequest request)
        {
        ...
        public bool TransacctionReverse(Models.Transaction request)
        {
